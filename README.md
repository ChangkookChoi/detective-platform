...
현재 저장소는 **핵심 MVP 기능 구현 단계**입니다. PostgreSQL 초기
...
구현했습니다. 공식 사무소 5곳에 이어 최신 공식 원문을 통과한 8곳의 소재지·
업무 분야·출처를 추가 검수하고 관리자 승인으로 공개해 현재 공개 업체는 총
14곳입니다. 신규 공식 홈페이지 10곳을 추가 조사해 최소 사실 필드가 일치한
6곳은 비공개 고위험 검수 후보로 등록했고, 차단·중복·DNS 실패·호스팅 만료가
있는 4곳은 등록하지 않았습니다. 운영 DB와 배포는 아직 준비 전입니다. 최신
진행 상황은 [docs/STATUS.md](docs/STATUS.md)를 기준으로 확인합니다.
