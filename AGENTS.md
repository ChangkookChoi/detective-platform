...
- 업체 데이터 확대 작업은 `docs/operations/OFFICE_DATA_EXPANSION.md`의 배치
  절차를 기본값으로 사용한다. 새 세션에서도 소규모 일회성 방식으로 되돌리지
  않는다.
- 기본 배치는 신규 도메인 20곳 이상을 발굴하고, 구조화된 manifest와
  `preflight-batch`로 기존 등록부·DB 중복, DNS/TLS, robots와 현재 HTTP 상태를
  한 번에 검사한다. 통과 수를 맞추기 위해 검증 기준을 낮추지 않는다.
- 서로 다른 host는 제한된 병렬 사전검증을 허용하지만, 같은 host 요청은
  순차 처리하고 robots의 요청 간격과 차단 지침을 우선한다.
- 실제 등록·승인은 SQL 직접 쓰기가 아니라 Clerk 인증 관리자 화면과 기존
  Server Action을 통과하는 재개 가능한 배치 실행 경로를 사용한다. 각 후보의
  공개 전 필수 확인, 감사 이력과 공개 상세 검증을 생략하지 않는다.
- 등록부에서 `manual_approved`인 출처와 운영 DB의 이름·전화·주소·출처 중복은
  자동 제외한다. `deferred` 재검사는 복구 근거를 manifest에 명시하며,
  `blocked`는 차단 사유가 해소됐다는 별도 사람 검토 없이는 재접근하지 않는다.
- 개발 DB 배치와 Production 승격은 분리한다. Production은 별도 승인, 백업과
  승격 검증이 있을 때만 묶음으로 처리한다.
- 이 작업 방식을 개선할 때는 같은 변경에서 이 절, 운영 runbook, 실행 도구와
  회귀 테스트를 함께 수정한다. 문서만 또는 일회성 스크립트만 바꾸고 영구
  절차가 갱신됐다고 간주하지 않는다.
