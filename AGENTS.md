...
- 1차 공개 범위는 서울·경기·인천 탐정사무소 약 100곳
- 비공개 후보 수집·중복 제거 목표는 전국 500곳 이상이며 공개 전 관리자 검수
- 외도·가족 문제 등 개인 고객 중심
- 지역 및 업무 분야별 업체 탐색
- 업체 상세 정보 제공
- 무료 전화 연결
...
- 실제 등록·승인은 SQL 직접 쓰기가 아니라 Clerk 인증 `/admin/reviews/batch`와
  Server Action을 통과하는 재개 가능한 배치 실행 경로를 사용한다. manifest의
  slug·최하위 지역·업무 분야·출처 유형을 후보에 함께 보존해 다시 입력하지
  않고, 운영자가 표에서 행별 값을 확인한 정상 건만 선택해 한 번에 승인한다.
  각 업체에는 독립된 검수 항목·승인 감사 이력·출처 근거가 남아야 하며 일부
  충돌은 다른 정상 건을 롤백하지 않고 해당 후보만 미처리 상태로 남긴다.
