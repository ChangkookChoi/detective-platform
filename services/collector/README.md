...
출처 등록과 실행 절차는 [수집기 운영 절차](../../docs/operations/COLLECTOR_RUNBOOK.md), 정책 경계는 [데이터 수집 정책](../../docs/operations/DATA_COLLECTION_POLICY.md)을 따릅니다. 현재 저장소에는 실제 출처가 아닌 [`sources.example.toml`](sources.example.toml)만 있습니다.
...
실제 출처 파일은 예시를 복사한 뒤 이용 조건과 robots를 사람이 확인하고 모든 placeholder를 승인된 값으로 바꿉니다. 설정 검증은 정책 필수 항목의 형식만 확인하며 법적·계약상 허용을 대신 판단하지 않습니다.
