...
PR #9에서는 read-only artifact API로 실행 전 400MiB 저장량 여유와 업로드 후
현재 run·attempt의 양의 크기·13~15일 보존을 검증하는 guard를 추가했다. 병합 후
수동 backup run `31667893789`이 사전 3개·182,895바이트와 사후
4개·243,860바이트를 확인하고 51초에 성공했다. 이 repository 단위 guard는
Packages와 계정 전체 공유 사용량을 대신하지 않으므로 Billing의 0원 초과 사용
중지와 포함 사용량 알림을 계속 유지한다.
