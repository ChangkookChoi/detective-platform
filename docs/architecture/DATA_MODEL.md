...
- `slug`는 공식 업체명과 최하위 소재 지역으로 결정론적으로 제안한다. 한글
  브랜드는 코드 기반 로마자화하고 일반 업종 표현은 관리형 영문 토큰으로
  정규화한다. 기존 업체 또는 미해결 후보와 기본 slug가 겹치면 검수 항목
  식별자 기반의 고정 suffix를 붙인다. 최종 값은 전역 고유하며 업체가 보관
  처리되어도 재사용하지 않는다.
...
서울·경기·인천 및 하위 행정구역의 가변 깊이 계층을 표현한다.
...
자유 텍스트 주소와 검색용 지역 분류를 분리한다. 자기 자신을 부모로 지정할 수 없고 계층 순환을 허용하지 않는다. 비활성 지역은 기존 업체 참조를 보존하되 신규 연결과 필터 노출에서 제외한다. 서울·경기·인천 이외 지역은 MVP seed에 포함하지 않는다.
