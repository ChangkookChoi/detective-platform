...
## 2026-08-20 NAVER API HUB 제한적 발견 파일럿

현재 상업 기능을 운영하지 않는 단계에서 `ADR-0010`의 제한을 적용해 NAVER API
HUB 지역 검색 기반 비공개 후보 발견 1차 파이프라인을 구현했다. 서버 전용 키,
실행당 요청 예산, 최대 2회 시도, Git 제외 Raw JSONL과 별도 filtered JSONL,
7일 만료 시각, 파일 권한 `0600`, 서울·경기·관련성·실행 내부·DB·출처 등록부
중복과 링크 유형의 사유 코드 필터를 적용했다. 필터 규칙 변경 시 API를 다시
호출하지 않고 같은 Raw 파일을 재사용하는 명령도 추가했다.

첫 실배치는 공개 업체가 없던 서울 관악·광진, 경기 광명·구리를 대상으로 검색어
2개씩 총 8회 호출해 Raw 40건을 비공개로 저장했다. 보수적으로 관련성 불확실을
자동 반려하지 않게 규칙을 조정한 당시 v1 재필터 결과는 `eligible` 1건,
`needs_review` 32건, `rejected` 7건이다. 공식 링크 없음 33건, 실행 내부 중복
6건, 관련성 재검토 10건이 핵심 패턴이었다. 실제 업체명·주소·URL은 AI 출력,
Git, 공개 DB와 Production에 전달하지 않았다. 지역 API만으로 공식 출처와
전화번호를 완성할 수 없으므로 웹문서 보강 또는 별도 공식 홈페이지 확인 단계는
약관·전문가 검토와 분리해 후속 결정한다. 개발 DB 64건·Production 32건은
변경하지 않았다.

후속 hardening에서 Raw v2를 제목·링크·분류·주소로 최소화하고 새 실행 전 만료
파일 자동 파기와 명시적 파기 명령, 만료 Raw 재필터 차단을 추가했다. 오해 소지가
있는 `eligible`은 v2에서 `source_check_required`로 바꾸고 모든 filtered 결과를
`source_verification=required`, `promotion_allowed=false`로 고정했다. 검색 결과를
manifest로 직접 변환하는 경로는 없으며 공식 홈페이지에서 독립적으로 확인한
값만 기존 사전검증·관리자 승인 경계로 보낼 수 있다.

같은 날 지역 Raw의 공식 링크 없음·비공식 링크 후보 중 중복을 제거한 10건을
웹문서 API로 10회 재검색해 제목·URL만 담은 Raw 45건을 비공개 저장했다. 결정론적
필터 결과는 `source_check_required` 7건, `needs_review` 29건, 기존 출처·실행 내
중복 반려 9건이다. 이 7개 URL을 원문 저장 없이 공개 IP·HTTPS·동일 사이트
redirect·robots·응답 상태·2.5MB 크기 제한으로 직접 점검해 본문 대조 대상으로
3건을 남겼고, robots 차단 1건과 응답 크기 초과 3건은 각각 차단·보류했다. 모든
결과는 부모 지역 Raw와 같은 2026-08-27 만료 시각과
`source_verification=required`, `promotion_allowed=false`를 유지한다. 실제 검색
결과나 페이지 원문은 AI·Git·DB·Production에 전달하지 않았고 개발 DB 64건·
Production 32건은 변경하지 않았다.

## 2026-08-21 NAVER 후보 발굴 확대와 검토 큐

개발 DB의 활성 최하위 지역 72개를 코드로 조회하고 `탐정사무소`, `흥신소`,
`민간조사`, `사설탐정`, `탐정업체` 다섯 검색어를 각각 독립 실행했다. 이번 확대
실행은 지역 API 360회로 Raw 1,526건을 확보했고, 기존 DB·출처 등록부·실행 내
중복과 지역·관련성 규칙을 먼저 적용했다. 공식 링크 보강이 필요한 고유 후보만
웹문서 API 444회로 재검색해 제목·URL만 담은 Raw 1,858건을 추가했다. 최초
파일럿까지 합친 현재 임시 보존분은 지역 368회·Raw 1,566건, 웹문서 478회·Raw
1,984건이다. 이는 호출 한도 확대를 결과 전체 저장 확대로 그대로 연결하지 않고
중복 이력으로 같은 후보의 재검색을 막은 결과다.

웹문서 필터를 통과한 URL 130건을 공개 IP·HTTPS·redirect·robots·응답 상태·
2.5MB 제한으로 점검했다. 본문 사실 대조 가능 84건, robots 차단 32건, 응답
문제 보류 12건, 정책 수동 확인 2건으로 분리했으며 차단은 우회하지 않았다.
허용된 페이지는 원문을 저장하지 않고 JSON-LD와 보이는 HTML에서 업체명·업무용
전화·주소 신호만 추출했다. 강한 일치 27건, 부분 일치 30건, 정보 부족 24건,
접근·인코딩 실패 3건이다. 최초 39곳 검토 큐는 단순 `탐정` 부분 문자열과
업체 사실 일치만 사용해 동물보육원·레미콘·추리게임 팝업·행정사 같은 무관
후보가 섞인 결함을 확인해 폐기 대상으로 전환했다. API를 재호출하지 않고 지역
Raw 1,566건을 v3 규칙으로 다시 분류하고 84개 공식 페이지의 현재 원문에서
탐정 업무 문구를 다시 추출했다. 업무 증거가 확인된 페이지는 23건이었다.

수정된 v2 검토 큐는 서울·경기, 탐정업 관련 상호 또는 분류, 비공식 host 제외,
공식 페이지의 강한 업체 사실 일치, 대표 전화, 탐정 업무 문구, 개발 DB 중복
부재를 모두 요구한다. 그 결과 최종 수동 검토 대상은 1곳, 추가 조사 대상은
23곳이며 알려진 무관 후보 4종은 두 큐 모두 0건이다. 최종 큐와 추가 조사 큐는
Git 제외 `0600` JSONL이고 각각 `review_status=pending`과
`review_status=research_required`, 모두 `promotion_allowed=false`다. 개발 DB
64건과 Production 32건에는 적재하거나 공개하지 않았다.

재실행 효율을 위해 웹 검색 배치 manifest에 후보 정체성 해시를 추가했다. 같은
업체가 다른 지역 질의나 검색어에서 다시 나타나도 보존기한 안에는 웹 API를
재호출하지 않으며, 결과가 0건인 후보도 처리 이력에 남긴다. 공식 출처 사실
파일을 업체 관련성·업무 증거·강한 사실 일치·전화·DB 중복까지 검사하고 검토와
추가 조사 큐로 나누는 `build-discovery-review-queue` 명령을 추가했지만 기존
운영 manifest로 자동 변환하거나 관리자 승인 없이 DB에 쓰는 경로는 만들지
않았다.

## 2026-08-21 공식 업무용 이메일 수집 기반

공식 홈페이지의 JSON-LD `email`, `mailto:`와 보이는 단일 이메일을 원문 저장
없이 추출하고 표시값·정규화값·`generic_business`/`unknown` 유형으로 분류하는
수집 기능을 추가했다. 이메일은 선택 정보이며 신규 수동 후보·일괄 후보·자동
변경 후보 모두 기존 관리자 승인 경계를 통과해야 한다. 누락이나 추출 실패는
기존 값을 삭제하지 않는다.

`offices`의 선택 이메일 필드와 공식 출처 `email` 근거, 별도
`office_email_marketing_consents` migration을 추가했다. 동의 테이블에 행이 없으면
발송 동의가 없는 것으로 해석하며 수집·후보 승인 과정은 동의 행을 자동 생성하지
않는다. 대량 메일 발송은 구현하지 않았다.

지속형 개발 DB에 additive migration을 적용한 뒤 공개 업체 64곳과 보류 후보
1건의 등록된 공식 출처 65곳을 정책·robots·요청 제한 안에서 순차 확인했다.
단일 이메일 후보 29곳, 공식 이메일 미발견 32곳, 복수 이메일 수동 선택 필요
2곳, 접근 실패 2곳으로 분리했다. 아직 업체가 없는 보류 후보 1곳은 기존 검수
결정과 섞지 않았고, 단일 후보가 확인된 기존 업체 28곳만 `field_change` 관리자
검수 큐에 등록했다. 무변경 dry-run에서 28건 모두 기존 미해결 항목으로 인식되어
중복 생성되지 않았고, 마케팅 발송 동의는 0건이다. 후보 파일은 Git 제외 `0600`,
7일 보존이며 Production Neon 32곳은 변경하지 않았다.

## 2026-08-21 전국 비공개 후보 500곳 목표 달성

서울·경기 공개 범위는 유지한 채 명시적 `--nationwide` 실행에서만 전국 후보를
수집하도록 지역 경계를 확장했다. 17개 광역 질의 결과의 주소에서 시·군·구
112개를 코드로 추출하고, 실행당 100회 요청 예산 안에서 분할 검색했다. 전국
지역 검색 13개 실행의 Raw 3,144건을 관련성·지역·기존 DB·출처 등록부·실행 및
이력 중복 규칙으로 처리한 결과 비반려 고유 후보 1,005곳을 확보해 비공개 후보
500곳 목표를 넘겼다. 이 수치는 검증 또는 공개 업체 수가 아니다.

공식 출처 확인 가능성이 높은 고유 후보 820곳만 웹문서 API로 보강해 Raw
2,962건과 공식 링크 후보 32건을 얻었다. 지역 API가 직접 제공한 공식 링크
후보 37건은 웹문서 API를 재호출하지 않고 같은 probe 단계로 보냈다. 직접 링크
37건은 본문 대조 가능 16건, 접근 차단 10건, 응답 보류 5건, 정책 수동 확인
6건으로 분리했고 차단을 우회하지 않았다. 웹문서·직접 링크의 사실 파일 37건을
통합한 최종 큐는 관리자 검토 후보 3곳, 추가 조사 후보 22곳이다. 나머지는 업무
증거·대표 전화·강한 사실 일치 부족, 공유 출처·지점 또는 중복 사유로 제외했다.

전국 수집값은 Git 제외 `0600` 파일로 7일만 보존하며 AI 입력, 관리자 운영
manifest, 개발·Production DB에 자동 전달하지 않는다. 개발 DB 공개 64곳,
Production 32곳은 그대로다. 전국 후보를 실제 관리자 검수에 넣으려면 전국 지역
seed와 비공개 discovery intake 변환을 별도 구현하고, 공식 출처와 최소 사실이
확인된 후보만 기존 승인 경계로 넘겨야 한다.

## 2026-08-25 관리자 공식 출처 증거 미리보기

검수 상세에 공식성·업체 동일성·핵심 정보·결정 순서 안내와 업체명·대표 전화·
선택 이메일·소재지의 후보값/출처 추출값 비교표를 추가했다. 비교 결과는 일치,
값 다름, 후보 또는 출처값 누락, 양쪽 모두 없음으로 구분하며 일치를 자동 승인
근거로 사용하지 않는다. 출처 host·유형·수집 시각·검증 근거 메모를 함께
표시하고 안전한 HTTP(S) 원문은 새 창으로 연다.

외부 홈페이지는 `iframe`으로 삽입하지 않아 프레임 차단을 우회하지 않고 외부
스크립트·광고·추적 코드를 관리자 화면에서 실행하지 않는다. 기존 최소 추출값과
원문 링크만 사용하므로 DB migration과 외부 페이지 전체 저장은 없다. 실제 Clerk
관리자 E2E 5건과 reviewer E2E 2건, lint와 webpack Production build를 통과했으며
임시 E2E DB 밖의 개발·Production 데이터는 변경하지 않았다.

대기열에는 `위험도`, `검수 종류`, `업체 · 발생 사유`, `등록 시각` 열 제목을
추가하고 모바일에서는 각 행 안에 같은 항목명을 표시했다. 위험도는 업체 품질이
아니라 잘못 반영했을 때의 영향도임을 설명하고, 내부 생성 사유 코드는 운영자가
읽을 수 있는 한국어 문장으로 변환한다. NAVER API 발견 파일은 운영 검수 DB와
분리되어 공식 출처·대표 전화·주소·동일성·소재 지역 확인과 명시적 intake 변환
전에는 대기열에 보이지 않는다는 안내도 화면에 추가했다. 아직 운영 업체와
연결되지 않은 신규 후보는 일반 문구 대신 제안된 업체명을 대기열에 표시한다.

## 2026-08-25 발견 파이프라인 자동화·재검증

지역 결과의 명시적 무관 문맥이 NAVER 관련 분류 하나 때문에 통과하지 않도록
관련성 규칙을 v4로 보강했다. 기존 서울·경기 Raw 2,141건과 전국 Raw 3,144건을
API 재호출 없이 이전 v3 결과와 비교한 결과 28건이 추가 반려됐고, 기존 반려가
다시 후보로 바뀐 건은 0건이며 직접 공식 링크 후보 147건은 유지됐다. 감사
보고서는 업체명·주소·URL·record ID 없이 상태와 사유 집계만 담는다.

최신 전국 100개 지역을 `탐정사무소` 한 검색어로 다시 호출해 Raw 489건을
확보했다. 실행 내 중복 190건과 무관 업종 73건 등을 코드로 제외해 직접 공식
링크 40건, 공식 출처 추가 검색 필요 191건으로 분리했다. 직접 링크는 웹문서 API를
호출하지 않았고, 추가 후보 100건의 웹문서 검색 Raw 427건에서는 공식 링크
후보가 4건만 남았다. 공식 링크 44건의 정책·네트워크 probe는 본문 확인 가능
10건, 수동 정책 확인 5건, 차단 26건, 보류 3건이었으며 차단은 우회하지 않았다.

본문 확인 10건은 강한 사실 일치 4건, 부분 일치 5건, 정보 부족 1건이고 공식
탐정 업무 증거는 6건이었다. 개발 DB 중복을 포함한 최종 비공개 분류는 검수 가능
1건, 추가 조사 6건, 중복·정보 부족 제외 3건이다. 추가 조사 사유 자동 라우팅을
구현해 신규 6건은 지점 충돌 수동 검토 4건·동일 도메인 보강 2건, 기존 전국
22건은 각각 11건·11건으로 나눴다. 공식 시작 페이지가 연결한 같은 도메인의
최대 3페이지를 다시 정책 검사하는 보강을 13건에 적용해 총 21페이지에서 전화·
이메일·주소·업무 신호만 확인했지만 모든 필수 사유를 해소한 후보는 0건이었다.
따라서 이 보강은 자동 승격 근거가 아니라 사람에게 넘길 사유를 줄이는 제한적
단계로 유지한다.

반복 절차는 `scripts/run-naver-discovery-cycle.sh` 한 명령으로 연결했고 활성 Raw
디렉터리 지역 구성, v3→v4 익명 집계 감사, 추가 조사 라우팅·동일 도메인 보강을
각각 독립 CLI로도 제공한다. 모든 신규 파일은 Git 제외 `0600`, 7일 보존,
`promotion_allowed=false`이며 개발 DB 64건·Production 32건은 변경하지 않았다.
...
럭스 승격 뒤 신규 공식 후보를 다시 조사해 현재 원문을 직접 확인할 수 없는
소나무·오케이·다해는 등록하지 않았습니다. 해담 탐정사무소는 HTTP 200 공식
원문에서 상호·사업자 표기·서울 서초 단일 주소·사업상 대표번호를 재확인하고,
robots 404 때문에 자동 수집에는 추가하지 않은 채 실제 Clerk 관리자 수동
...
이후 Production 작업은 보류하고 개발 DB 데이터 확대를 재개했습니다. 신규 후보
여섯 곳의 robots를 홈페이지보다 먼저 확인해 소나무·오케이는 DNS 실패로
미등록했고, 정암 공인탐정·민간조사 서현·베테랑탐정사무소·더PIA탐정사무소는
현재 공식 원문에서 서울 소재 한 개 사무소의 상호·대표번호·주소와 직접
뒷받침되는 업무 분야를 확인했습니다. 실제 Clerk 관리자 화면과 Server Action을
거쳐 네 곳을 제안값 그대로 승인·공개해 지속형 개발 DB는 36곳입니다. 네 곳의
...
이후 배치 manifest에 이미 있는 slug·최하위 지역·업무 분야·출처 유형을 업체마다
관리자 폼에 다시 입력하던 병목을 제거했습니다. `/admin/reviews/batch`에서
manifest와 24시간 이내 preflight를 한 번 제출해 비공개 후보를 일괄 생성하고,
근거 메모와 행별 값을 대조한 정상 후보만 선택해 한 번 승인할 수 있습니다.
기존 미처리 동일 후보는 재사용하고 정확히 공개된 후보는 건너뛰며, 업체별 기존
승인 트랜잭션·감사 이력·출처 근거는 그대로 유지합니다. 일부 충돌은 다른 성공
건을 되돌리지 않고 해당 후보만 미처리로 남기도록 DB 계약을 검증했습니다.
이 기능 구현에서는 개발·Production 실제 업체 수를 변경하지 않았습니다.
2026-08-14에는 공식·협회 공개 후보를 추가 발굴하되 제3자 디렉터리 하나만으로
중요 필드를 확정하지 않는 정책을 적용해 협회 프로필 단독 후보를 제외했습니다.
첫 공식 후보 12곳의 네트워크 재검사에서 DNS·연결 실패 7곳과 AI robots 명시
차단 1곳을 제외하고, 현재 공식 원문과 정책을 통과한 코난·리더스·대한특수탐정
더원 수원지점·서울 종로 광역센터 4곳만 영구 관리자 배치로 승인·공개했습니다.
기존 무궁화 업체와 전화·주소가 같은 별도 도메인을 확인하면서 `+82`/국내 전화
표기와 선행 우편번호·주소 구두점을 동일 키로 비교하도록 중복 검사를 보강해
재등록을 차단했습니다. 신규 4곳의 제출자·처리자, 대표 출처·필드 근거와 공개
상세 HTTP 200·전화/출처 링크·브라우저 오류 0건을 확인해 개발 DB는 45곳이며,
Production Neon은 32곳에서 변경하지 않았습니다.
이어 공식 지점 목록을 직접 대조해 디테일탐정사무소 경기지사와 대한특수탐정
더원 서울서초지점을 기존 본사·센터와 구분하고, 서울 서초 단일 사무소의 공식
최소 사실을 확인한 픽서컴퍼니를 더했습니다. 공식 URL이나 대표번호를 공유해도
정확한 주소가 다른 지점임을 공식 원문에서 확인하고 명시적으로 검토한 경우만
허용하도록 영구 배치 사전검증을 개선했습니다. 같은 출처·주소 조합과 기존
주소·slug 중복은 계속 차단합니다. 최종 3건은 preflight 3/3과 Clerk 관리자
일괄 등록·승인, 공개 상세·감사·근거 검증을 통과해 개발 DB는 48곳이며,
Production Neon은 32곳에서 변경하지 않았습니다.
...
- 다음 신규 공식 후보 조사에서 소나무·오케이는 DNS 실패, 다해는 직접 홈페이지
  응답이 32바이트 식별자뿐이라 과거 검색 캐시를 승인 근거로 사용하지 않고
  미등록. 해담은 현재 공식 HTML 114,982바이트와 약관·최소 사실 필드를 확인하고
  robots 404 경계에서 자동 수집 없이 수동 후보로 처리
- 실제 Clerk testing token과 allowlist 관리자 세션의 신규 후보·상세 폼으로
  해담을 제안값 그대로 승인·공개. 서울 서초와 가족 문제·증거·사실 확인을
  연결하고 유효한 제출자·처리자, 대표 출처 1건·근거 5건·업무 분야 2건,
...
- 신규 공식 후보 여섯 곳의 robots·현재 원문과 기존 공개 32곳 중복을 확인.
  소나무·오케이는 DNS 실패로 미등록하고 정암·서현·베테랑·더PIA 네 곳만 실제
...
- 업체별 등록·승인 폼 반복을 제거한 `/admin/reviews/batch` 구현. manifest와
  preflight 일괄 제출, 제안 메타데이터 보존·상세 자동 채움, 정상 건 선택 승인,
  미처리 재사용·기승인 건너뛰기·부분 실패 재개와 업체별 감사/근거 유지를 DB
  계약·lint·production build로 검증. 실제 개발 DB·Production 업체 수는 미변경
- 2026-08-14 공개 후보를 추가 발굴하고 협회 디렉터리 단독 근거·DNS/연결 실패·
  AI robots 차단 후보를 제외. 기존 무궁화 업체와 같은 전화·주소의 별도 도메인을
  `+82` 전화·우편번호 포함 주소 정규화로 자동 중복 탐지하도록 보강하고 단위
  테스트 통과. 코난·리더스·대한특수탐정 더원 수원·서울 종로 4곳은 preflight
  4/4와 Clerk 관리자 배치 E2E 2/2를 통과해 승인·공개. 개발 DB 45건·`approved`
  44건·`approved_with_edits` 1건·`on_hold` 1건·`rejected` 1건·`pending` 0건,
  Production Neon 32건 미변경
- 2026-08-14 공식 지점 후보 7건을 재검사해 DNS 실패 3건·호스팅 만료 1건을
  제외하고 디테일 경기지사·더원 서울서초지점·픽서컴퍼니 3건을 preflight
  3/3과 Clerk 관리자 배치 E2E 2/2로 승인·공개. 공식 원문의 별도 주소를 사람이
  명시적으로 확인한 지점만 공유 URL·전화 중복을 허용하고 주소·slug 중복은
  계속 차단하도록 사전검증·관리자 계약과 단위 테스트를 갱신. 개발 DB 48건·
  `approved` 47건·`approved_with_edits` 1건·`on_hold` 1건·`rejected` 1건·
  `pending` 0건과 무변경 재실행 2/2를 확인, Production Neon 32건 미변경
- 2026-08-14 다음 공식 후보 묶음에서 강동·안양·분당·김포의 실제 사무소
  4건을 추가 승인·공개. 위너·우람한은 DNS/연결 실패, 호시탐탐은 만료 화면,
  오케이는 DNS 실패로 제외하고 대성기획 서울지사는 기존 공개 업체와 같은
  호실이라 중복 제외. 최종 두 배치는 preflight 3/3·1/1과 Clerk 관리자 배치
  E2E 각 2/2를 통과해 개발 DB 52건·`approved` 51건·`pending` 0건으로 확대,
  Production Neon 32건 미변경
- 2026-08-16 검증된 공식 지점망을 우선하는 수집으로 더원 성남 글로벌
  보안센터와 119탐정센터 서울지사 2건을 추가 승인·공개. TLS·DNS 장애,
  기존 주소·전화 중복, 제3자 디렉터리 단독 후보와 상세 주소 없는 지점은 제외.
...
1. 초기 업체 약 100곳 확대를 계속한다. 공식 운영 주체·한 개 사무소·최소
   사실 필드와 업무 분야를 모두 확인한 건은 같은 위임 작업 안에서 등록→승인→
   공개 검증까지 완료하고 불확실한 건만 보류한다. 다음 배치는 경기의 아직
   적은 시·군과 서울의 미충족 구를 우선하되 검색 색인만으로 등록하지 않는다.
2. 저장량·보존 guard가 적용된 후속 02:23 KST 예약 백업의 연속 성공과 실제
...
5. 개발 DB의 신규 공개분은 나중에 별도 사용자 승인·사전 암호화 백업·최소
   권한 증분 작업으로 Production Neon에 반영한다.
...
### 2026-08-14 개선 배치 05·06 지역 공백 확대

- 서울·경기의 공개 업체 48건 분포를 먼저 확인하고 기존 과밀 지역보다 서울
  미충족 구와 경기의 업체가 적은 시를 우선했다. 공식 후보 8건을 비교해 DNS
  또는 연결 실패인 위너·우람한, 이미 만료 화면이 확인된 호시탐탐, DNS 실패가
  반복된 오케이는 등록하지 않았다.
- 대성기획 서울지사의 공식 주소는 기존 공개 내일탐정사무소와 같은 건물·동·층·
  열·호로 확인되어 브랜드명이 달라도 같은 사무소 중복으로 제외했다. 검색
  색인만 남은 후보, 상세 주소가 없는 서비스 지역, 제3자 디렉터리 단독 후보도
  승인 근거로 사용하지 않았다.
- 명탐정마당발 수도권사무소·경기권사무소와 대성기획 경기지사는 공식 페이지가
  각 지점의 정확한 주소를 직접 구분한 것을 확인했다. 최종 manifest 3건은
  preflight 3/3과 Clerk Development allowlist 관리자 배치 E2E 2/2를 통과해
  강동·안양 만안·성남 분당에 승인·공개됐다.
- 프로탐정은 공식 홈페이지의 `찾아오시는 길`에 표시된 김포 방문 사무소 주소와
  대표전화·사업자번호·업무 분야를 구분해 확인했다. preflight 1/1과 같은 관리자
  배치 E2E 2/2로 승인·공개했다.
- 독립 DB 조회에서 최종 `approved` 51건·`approved_with_edits` 1건·`on_hold`
  1건·`rejected` 1건·`pending` 0건, 지속형 개발 DB 공개 업체 52건을 확인했다.
  신규 상세의 HTTP 200·표시값·전화/출처 링크와 브라우저 오류 0건은 배치 E2E가
  검증했다. Production Neon은 사용자 요청대로 32건에서 변경하지 않았다.

### 2026-08-16 개선 배치 01 공식 지점망 확대

- 공식 지점망을 먼저 묶음 확인해 더원 성남 글로벌 보안센터와 119탐정센터
  서울지사를 신규 후보로 선정했다. 더원은 기존 종로·서초·수원과 다른 성남
  수정구 주소를, 119는 서울지사 전용번호와 강남 상세 주소를 확인했다.
- 119 공식 화면의 `남부수환로` 표기는 실제 도로명 검색 결과와 교차 확인해
  `남부순환로`로 정규화했다. 상세 주소가 없는 119 수원지사, 공식 원문에
  접근할 수 없는 디파인·솔리스와 기존 디테일 경기지사와 같은 주소·전화인
  홍길동 후보는 등록하지 않았다.
- manifest 2건은 정책 preflight 2/2와 Clerk Development allowlist 관리자
  배치 E2E 2/2를 통과했다. 공개 상세 HTTP 200, 상호·전화·주소·대표 출처,
  전화 링크와 브라우저 오류 없음까지 확인했다.
