...
접근·인코딩 실패 3건이며, 같은 업체를 정체성 해시로 다시 합친 수동 검토 큐는
39곳(강한 일치 17곳·부분 일치 22곳)이다. 큐는 Git 제외 `0600` JSONL이고 모두
`review_status=pending`, `promotion_allowed=false`다. 개발 DB 64건과 Production
32건에는 적재하거나 공개하지 않았다.
...
파일을 강한/부분 일치만으로 중복 합치는 `build-discovery-review-queue` 명령도
추가했지만 기존 운영 manifest로 자동 변환하거나 관리자 승인 없이 DB에 쓰는
경로는 만들지 않았다.
