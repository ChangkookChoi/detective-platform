...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 21건·`approved_with_edits` 1건·`on_hold`
  1건·`pending` 3건, 공개 업체 23건
...
등록 경로로 비공개 후보를 만들었습니다. 같은 운영 주체인 바른기획·VIP의 별도
브랜드 도메인, 인천 소재 리셋과 만료 화면이 유지된 호시탐탐은 등록하지
않았습니다. 수동 후보는 총 26건이고 신규 3건만 `pending`이며 공개 업체는
23곳 그대로입니다.
...
1. 신규 대기 후보 DSI·에이원·넘버원의 공식 원문을 승인 직전에 재확인하고,
   최하위 소재지·업무 분야·대표 출처를 지정해 승인·보류·반려 결정
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자 테스트
  ...
  수동 후보 26건 중 기존 23건은 testing token과 일회성 sign-in token으로 지역
  2단계 선택·출처 유형·업무 분야·승인 또는 보류·공개 상세까지 통과했고, 신규
  3건은 승인 전 `pending` 상태다. 이는 Google 로그인
  UI와 관리자 Google 계정의 2단계 인증 수행 자체를 증명하지 않으며 reviewer
  전용 권한의 추가 브라우저 검증은 아직 남아 있다.
