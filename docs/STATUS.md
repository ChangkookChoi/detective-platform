...
- 단계: 핵심 MVP 기능 구현
- 배포 상태: 미배포
- 데이터 상태: 실제 파일럿 검수 후보 1건 `on_hold`, 공개 업체 0건
...
저장했으며, 감사 처리자 ID 일치와 운영 업체 0건을 확인했습니다. 추가 공식
...
1. `jsonld-v1` 주소 중복 결함이 있는 보류 후보를 반려하고 `jsonld-v2`로
   개발 DB 파일럿을 다시 실행해 교정된 신규 후보 생성 확인
2. 교정된 파일럿 제안값과 공식 원문·지역·업무 분야를 사람이 대조하고 승인
   또는 보류 결정
...
- 등록 파일럿 한 곳은 실제 이용 조건 표시·robots·구조를 확인하고 최소 권한
  ...
  보류했으며 교정된 `jsonld-v2` 재수집과 사람의 최종 승인, 운영 DB 자격
  증명·네트워크 경계, 예약 실행과 알림은 아직 검증하지 않았다.
