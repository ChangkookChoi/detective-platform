...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 공개 탐색과 관리자 검수 흐름을 구현하고 있습니다. 데이터 계층, 홈·목록·상세, Clerk 인증 검수, 개인정보 최소화 일별 집계, 정책 기반 Python 수집기, 신규·변경·정정 후보 승인까지 구현했습니다. 공식 홈페이지 후보 네 곳의 정책·robots·구조를 확인해 한 곳의 단일 페이지·사실 필드 파일럿 정책을 등록하고 DB 비저장 실페이지 추출까지 검증했습니다. PostgreSQL 17 합성 데이터로 수집·정정 후보부터 공개 전환의 원자성과 실패 롤백을 통합 검증했으며, 지속형 개발·운영 DB, 실제 업체 데이터 적재와 운영 인증 키는 준비 전입니다.
...
1. 지속형 개발 DB와 수집기 최소 권한 역할을 준비하고 등록 파일럿 한 건을 적재해 관리자 검수까지 확인
2. 운영 Clerk 프로젝트·MFA·관리자 계정을 준비해 실제 인증 흐름 검증
3. 초기 업체 약 100곳의 출처 확인 및 수동 검수
4. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트 기반 출시 검증
...
- 등록 파일럿 한 곳은 실제 이용 조건 표시·robots·구조와 비저장 네트워크 추출을 확인했지만 사이트 운영자의 별도 서면 허락은 받지 않았다. 실제 DB 적재, 관리자 승인, 운영 네트워크·최소 권한 DB 역할, 예약 실행과 알림은 아직 검증하지 않았다.
