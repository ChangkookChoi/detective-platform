...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 13건·`pending` 0건, 공개 업체 14건
...
1. 초기 업체 약 100곳의 출처 확인 및 수동 검수. 공식 운영 주체·한 개 사무소·
   최소 사실 필드를 최신 원문에서 확인한 후보만 비공개 검수 대기열에 등록
