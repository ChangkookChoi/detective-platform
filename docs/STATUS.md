...
저장했으며, 감사 처리자 ID 일치와 운영 업체 0건을 확인했습니다. 주소
지역명 중복 정규화를 `jsonld-v2`로 교정해 최소 권한 재수집을 실행하고
교정된 `pending` 후보 1건을 추가했으며 여전히 운영 업체 0건입니다. 추가 공식
홈페이지 후보를 두 차례에 걸쳐 추가 조사했고, 그중 9곳을 자동 수집 없이
관리자가 재확인할 `manual_candidate` 준비 묶음으로 분류했습니다. 인증
운영자가 공식 URL과 최소 사실 필드만 고위험 비공개 후보로 등록하는 수동
유입 경로를 구현했습니다. 운영 DB와 배포는 아직 준비 전입니다.
...
- 같은 공식 출처 URL의 `pending`·`on_hold` 신규 후보 중복 방지와 입력
  검증, 제출만으로 운영 업체가 생성되지 않는 경계를 PostgreSQL에서 검증
...
- 같은 공식 출처 URL의 중복 수동 등록 시 기존 검수 ID를 반환하고 새 후보를
  만들지 않은 채 해당 상세 화면으로 안내하도록 개선
- URL 해시 조각이 달라도 같은 공식 출처로 정규화되어 중복 차단되는지
  PostgreSQL 통합 검증 추가
...
- 기존 후보를 포함해 공식 최소 사실 필드는 확인했지만 자동 수집이 부적합한
  9곳을 `manual_candidate`로 분리하고 입력 직전 재확인 조건을 문서화
- 수동 후보 등록에서 공식 운영 주체·정확한 한 개 사무소와 민감정보 미포함
  확인을 HTML뿐 아니라 서버 도메인 규칙으로 강제하고 누락 거부 검증 추가

## 다음 작업 후보

1. 로그인한 브라우저에서 `jsonld-v1` 주소 중복 결함이 있는 보류 후보를
   결함 사유로 반려
2. 교정된 `jsonld-v2` 제안값과 공식 원문·지역·업무 분야를 사람이 대조하고 승인
   또는 보류 결정
3. `manual_candidate` 9곳 중 정확한 단일 사무소 주소가 재확인된 1~2곳을
   수동 후보 화면으로 등록하고 브라우저 폼·중복 안내·감사 정보 표시 확인
4. 초기 업체 약 100곳의 출처 확인 및 수동 검수
5. 운영 DB·백업·네트워크·예약 수집·알림을 준비
6. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트 기반 출시
   검증
...
- 등록 파일럿 한 곳은 실제 이용 조건 표시·robots·구조를 확인하고 최소 권한
  로컬 개발 DB에 비공개 검수 후보 한 건을 적재했지만 사이트 운영자의 별도
  서면 허락은 받지 않았다. 최초 `jsonld-v1` 후보는 주소 지역명이 중복되어
  보류했고 교정된 `jsonld-v2` 후보를 별도로 생성했지만, 기존 후보 반려와
  교정 후보의 사람 최종 승인, 운영 DB 자격 증명·네트워크 경계, 예약 실행과
  알림은 아직 검증하지 않았다.
...
- 공식 출처 수동 후보 등록은 lint, production build와 공식성·민감정보
  확인을 포함한 도메인·PostgreSQL 통합 검증을 통과했지만 실제 로그인
  브라우저에서의 폼 제출·중복 안내와 접근성은 아직 시각 검증하지 않았다.
