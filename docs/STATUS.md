...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 공개 탐색과 초기 관리자 검수 흐름을 구현하고 있습니다. 데이터 계층, 공개 전환, 홈·목록·상세, Clerk 인증 검수 흐름에 이어 상세 조회·전화 클릭의 개인정보 최소화 일별 집계를 구현했으며 PostgreSQL 17 합성 데이터로 API부터 중복·속도 제한·집계까지 통합 검증했습니다. 지속형 개발·운영 DB, 운영 인증 키와 실제 업체 데이터는 준비 전입니다.
...
1. 수집기 어댑터, 변경 감지, 재시도 정책 구현
2. 신규 수집 후보의 운영 업체 생성과 수정 후 승인 편집 흐름 구현
3. 정보 오류 신고·정정 요청 진입점과 운영 처리 흐름 구현
