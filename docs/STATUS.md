...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 요구사항과 운영 원칙을 수립하고 있습니다. Next.js App Router 웹과 Python 3.13/`uv` 수집기 골격은 생성되어 있지만, 데이터베이스 연결과 사용자 기능은 구현 전입니다.
...
1. 초기 업체 표본으로 지점·공유 전화번호·지역 계층 예외 검증
2. PostgreSQL ORM/query builder 및 migration 도구 선정
3. 지역·업무 분야 seed와 초기 스키마 migration 구현
