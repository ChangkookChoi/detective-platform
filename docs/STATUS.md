...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 요구사항과 운영 원칙을 수립하고 핵심 공개 탐색 흐름을 구현하고 있습니다. 초기 스키마와 서울·경기 전체 시·군·구 seed, 공개 전환 규칙과 공개 업체 조회 repository, 홈·목록·상세 화면을 구현했으며 PostgreSQL 17 합성 데이터로 서버 렌더링 응답까지 통합 검증했습니다. 지속형 개발·운영 DB 연결과 관리자 기능은 구현 전입니다.
...
1. 관리자 인증과 검수 대기열에 공개 전환 유스케이스 연결
2. 조회·전화 버튼 클릭 이벤트의 개인정보 없는 최소 집계 구현
