...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 13건·`pending` 6건, 공개 업체 14건
...
1. 신규 수동 후보 6건의 공식 원문을 다시 대조해 최하위 소재 지역·관리형 업무
   분야·대표 출처 유형을 지정하고 승인·보류·반려 결정. 특히 복수 연락처와
   협회·브랜드 표기는 제안값을 그대로 신뢰하지 않고 공개 상세까지 별도 확인
2. 초기 업체 약 100곳을 향한 다음 공식 출처 묶음 조사와 수동 검수. 공식 운영
   주체·한 개 사무소·최소 사실 필드를 최신 원문에서 확인한 후보만 등록
3. 다해 화성 본사의 접속 복구와 명진 공식 주소 정합성을 나중에 재확인하고,
   복구·정정된 경우에만 최소 사실 필드를 처음부터 다시 검수
4. 리앤장 DNS와 호시탐탐 홈페이지 복구 여부를 나중에 재확인하고, 복구된
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자
  ...
  DB의 실제 후보 13건도 testing token과 일회성 sign-in token으로 지역 2단계
...
- 같은 날 추가 조사한 10곳 중 6곳은 현재 공식 원문의 최소 사실 필드만 확인해
  비공개 후보로 등록했다. 아직 소재 지역·관리형 업무 분야·대표 출처와 공개
  적합성을 승인한 상태가 아니며, 협회 서비스와 별도 브랜드 표기도 승인 전에
  다시 확인해야 한다.
