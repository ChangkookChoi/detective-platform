...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 공개 탐색과 관리자 검수 흐름을 구현하고 있습니다. 데이터 계층, 홈·목록·상세, Clerk 인증 검수, 개인정보 최소화 일별 집계, 정책 기반 Python 수집기, 신규·변경·정정 후보 승인까지 구현했습니다. 지속형 PostgreSQL 17 로컬 개발 DB와 수집기 최소 권한 역할을 준비하고 공식 홈페이지 한 곳의 단일 페이지·사실 필드 파일럿을 `pending` 신규 검수 후보 한 건으로 적재했습니다. Clerk Hobby Development `test` 키와 관리자 한 명의 역할 설정은 사전검증을 통과했으며 로그아웃 관리자 경로의 로그인 리디렉션까지 확인했습니다. 운영 업체 행은 생성하지 않아 비공개 경계를 유지했으며, 실제 브라우저 로그인·검수, 운영 DB와 배포는 확인 전입니다.
...
1. 사용자 브라우저에서 Clerk Google 로그인을 완료하고 파일럿 검수 대기열·상세 화면과 처리자 ID 확인
...
- Clerk Hobby Development 키와 실제 관리자 역할 설정은 사전검증하고 로그아웃 리디렉션을 확인했지만, 사용자 브라우저의 Google 로그인 세션과 관리자 대기열 접근은 아직 검증하지 않았다. Clerk Hobby는 애플리케이션 수준 MFA를 제공하지 않으므로 관리자 Google 계정의 2단계 인증·패스키 활성화를 운영 통제로 사용하며 Clerk가 해당 수행 여부를 기술적으로 증명하지는 않는다.
