...
1. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트를 기준으로
   출시 준비 격차를 점검하고, 로컬에서 결정 가능한 결함과 reviewer 전용 권한·
   관리자 보류/공개 승인 분기 E2E를 우선 보강
2. 운영 PostgreSQL 공급자·네트워크·자격 증명 선택에 필요한 요구사항과 선택지를
   정리하고, 공급자 결정 후 실제 자동 백업·시점 복구와 RPO·RTO를 격리 복원으로
   검증
3. 초기 업체 약 100곳 확대는 출시 기반 작업 뒤 재개. 공식 운영 주체·한 개
   사무소·최소 사실 필드와 업무 분야를 모두 확인한 건은 같은 위임 작업 안에서
   등록→승인→공개 검증까지 완료하고 불확실한 건만 보류
4. 고려 공식 사이트의 `a동 720`·`B동 720호` 주소 충돌이 정정되는지 나중에
   재확인하고, 하나의 공식 주소가 추가 근거와 일치할 때만 보류 후보 재검수
5. 다해 화성 본사의 접속 복구와 명진 공식 주소 정합성을 나중에 재확인하고,
   복구·정정된 경우에만 최소 사실 필드를 처음부터 다시 검수
6. 리앤장 DNS와 호시탐탐 홈페이지 복구 여부를 나중에 재확인하고, 복구된
