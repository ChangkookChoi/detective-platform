...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 요구사항과 운영 원칙을 수립하고 PostgreSQL 데이터 계층을 구현하고 있습니다. Next.js App Router 웹과 Python 3.13/`uv` 수집기 골격, Drizzle 기반 초기 스키마와 seed를 PostgreSQL 17에서 통합 검증했지만 지속형 개발·운영 DB 연결과 사용자 기능은 구현 전입니다.
...
1. 전체 서울·경기 하위 행정구역 seed를 공신력 있는 기준으로 확장
2. 공개 업체 조회 repository와 공개 상태 제약 유스케이스 구현
3. 업체 목록·상세 화면과 지역/업무 분야 필터 구현
4. 관리자 인증과 검수 대기열 구현
5. 수집기 어댑터, 변경 감지, 재시도 정책 구현
6. 조회·전화 버튼 클릭 이벤트 집계 구현
7. 초기 업체 약 100곳의 출처 확인 및 수동 검수
8. 접근성, SEO, 보안, 운영 체크리스트 기반 출시 검증
