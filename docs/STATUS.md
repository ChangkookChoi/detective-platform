...
홈페이지 후보 다섯 곳은 현재 지원 JSON-LD가 없어 자동 수집에서 보류했고,
인증 운영자가 공식 URL과 최소 사실 필드만 고위험 비공개 후보로 등록하는
수동 유입 경로를 구현했습니다. 운영 DB와 배포는 아직 준비 전입니다.
...
2. 교정된 `jsonld-v2` 제안값과 공식 원문·지역·업무 분야를 사람이 대조하고 승인
   또는 보류 결정
3. 보류한 공식 홈페이지 다섯 곳을 수동 후보 화면으로 소량 등록하고 실제
...
- 공식 출처 수동 후보 등록은 lint, production build와 도메인·PostgreSQL
  통합 검증을 통과했지만 실제 로그인 브라우저에서의 폼 제출·중복 안내와
  접근성은 아직 시각 검증하지 않았다.
