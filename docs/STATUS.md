...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  ...
  1건·`pending` 0건, 지속형 개발 DB 공개 업체 31건·Production Neon 30건
...
지속형 개발 DB는 공개 업체 31곳이며 Production Neon은 다음 공개 데이터 승격
전까지 기존 30곳을 유지합니다.
...
불일치 rollback·비공개 데이터 격리와 최초 bootstrap 회귀를 통과했으며, 실제
Neon 30→31 반영은 owner direct 자격 증명과 최신 백업을 준비한 뒤 수행합니다.
...
4. 신규 공개 1건을 증분 승격하기 전에 최신 Neon 암호화 백업 성공과 migration
   owner direct 자격 증명을 준비한다. `dry-run`에서 대상 30건·신규 1건을 확인한
   뒤 실제 반영, 대상 31건·신규 0건 재실행과 runtime·공개 상세를 검증
5. 초기 업체 약 100곳 확대를 계속한다. 공식 운영 주체·한 개 사무소·최소
...
- 2026-08-13 추가 공식 출처 조사에서는 접근 정책을 확인할 수 없거나 명시적으로
  ...
  DB의 31번째 업체이며 Production Neon에는 아직 승격하지 않았다.
