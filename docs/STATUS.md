...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 공개 탐색과 초기 관리자 검수 흐름을 구현하고 있습니다. 데이터 계층, 공개 전환 규칙, 홈·목록·상세에 이어 Clerk 인증 경계와 역할 제한, 검수 대기열·상세·승인·보류·반려를 구현했으며 PostgreSQL 17 합성 데이터로 상태 변경과 감사 이력을 통합 검증했습니다. 지속형 개발·운영 DB, 운영 인증 키와 실제 업체 데이터는 준비 전입니다.
...
1. 조회·전화 버튼 클릭 이벤트의 개인정보 없는 최소 집계 구현
2. 수집기 어댑터, 변경 감지, 재시도 정책 구현
3. 신규 수집 후보의 운영 업체 생성과 수정 후 승인 편집 흐름 구현
4. 정보 오류 신고·정정 요청 진입점과 운영 처리 흐름 구현
5. 운영 Clerk 프로젝트·MFA·관리자 계정과 지속형 DB 준비 후 실제 인증 흐름 검증
6. 초기 업체 약 100곳의 출처 확인 및 수동 검수
7. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트 기반 출시 검증
