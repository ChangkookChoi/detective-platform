...
않았습니다. 수동 후보는 총 26건이며 대기 후보는 0건, 공개 업체는 26곳입니다.
...
1. 초기 업체 약 100곳을 향한 다음 공식 출처 묶음 조사. 공식 운영 주체·한 개
   사무소·최소 사실 필드를 최신 원문에서 확인한 후보만 비공개 등록
2. 고려 공식 사이트의 `a동 720`·`B동 720호` 주소 충돌이 정정되는지 나중에
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자 테스트
  ...
  수동 후보 26건은 testing token과 일회성 sign-in token으로 지역 2단계 선택·
  출처 유형·업무 분야·승인 또는 보류·공개 상세까지 통과했다. 이는 Google 로그인
