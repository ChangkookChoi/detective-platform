...
1. Clerk 개발 프로젝트·Restricted mode·필수 MFA·관리자 계정과 키를 준비해 실제 인증 흐름과 파일럿 검수 상세 화면 확인
...
- Clerk 개발·운영 키와 실제 관리자 계정이 없어 로그인 UI, 세션과 역할 allowlist의 실제 공급자 연동은 아직 검증하지 않았다. 키 모드·경로·역할 ID 사전검증과 권한 판정·검수 데이터 흐름은 합성 설정·ID·데이터로 검증했다.
