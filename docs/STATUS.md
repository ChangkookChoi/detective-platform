...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 24건·`approved_with_edits` 1건·`on_hold`
  1건·`pending` 4건, 공개 업체 26건
...
1. 새 `pending` 4건의 공식 원문을 승인 직전에 다시 확인하고, 최하위 소재지·
   관리형 업무 분야·대표 출처 유형을 보수적으로 지정해 승인·보류·반려
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자 테스트
  ...
  수동 후보 30건 중 이전 26건은 testing token과 일회성 sign-in token으로 지역
  2단계 선택·출처 유형·업무 분야·승인 또는 보류·공개 상세까지 통과했다. 신규
  4건은 같은 공식 인증 경로의 등록과 비공개 경계까지만 통과해 승인 검수가
  남아 있다. 이는 Google 로그인
