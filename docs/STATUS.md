...
7. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트 기반 출시
   ...
   실제 로그인 관리자의 수동 후보 등록·중복 안내·검수 결정을 자동화한 E2E 추가
...
- Playwright E2E는 DB 비의존 공개·SEO·로그아웃 인증 12건과 임시 PostgreSQL
  기반 공개 사용자 전체 흐름 1건을 production 서버에서 검증한다. 실제 Clerk
  로그인 세션을 자동 준비하지 않으므로 관리자 수동 후보 폼·중복 안내·검수
  결정은 아직 자동 브라우저 검증 범위가 아니다.
