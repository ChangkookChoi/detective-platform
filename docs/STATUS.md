...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 17건·`approved_with_edits` 1건·`on_hold`
  1건·`pending` 4건, 공개 업체 19건
...
상세를 통과해 현재 공개 업체는 19곳입니다. 이어 공식 홈페이지 6곳을 조사해
흥신소 굿탐정 화성 본사·굿파트너·한마음·착한탐정 4곳을 같은 관리자 경로로
비공개 고위험 후보 등록했습니다. 충남 천안 소재 공존과 같은 공식 도메인에서
운영 법인·사업자번호가 충돌하는 LIRA는 등록하지 않았습니다.
...
1. 신규 대기 후보 4건의 공식 원문을 승인 직전에 다시 열어 상호·전화·주소,
   최하위 소재 지역과 직접 명시된 관리형 업무 분야를 대조하고, 대표 출처를
   `official_website`로 지정해 승인·보류·반려. 제안값을 추측해 보정하지 않음
2. 초기 업체 약 100곳을 향한 다음 공식 출처 묶음 조사. 공식 운영 주체·한 개
   사무소·최소 사실 필드를 최신 원문에서 확인한 후보만 비공개 등록
3. 고려 공식 사이트의 `a동 720`·`B동 720호` 주소 충돌이 정정되는지 나중에
   재확인하고, 하나의 공식 주소가 추가 근거와 일치할 때만 보류 후보 재검수
4. 다해 화성 본사의 접속 복구와 명진 공식 주소 정합성을 나중에 재확인하고,
   복구·정정된 경우에만 최소 사실 필드를 처음부터 다시 검수
5. 리앤장 DNS와 호시탐탐 홈페이지 복구 여부를 나중에 재확인하고, 복구된
   경우에만 운영 주체·대표번호·사무소별 주소를 처음부터 다시 검수
6. 운영 PostgreSQL 공급자·네트워크·자격 증명을 결정하고 실제 자동 백업,
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자 테스트
  ...
  수동 후보 23건 중 앞선 19건은 testing token과 일회성 sign-in token으로 지역
  2단계 선택·출처 유형·업무 분야·승인 또는 보류·공개 상세까지 통과했다. 신규
  4건은 등록·제출자·비공개 경계까지만 통과했고 승인 검수는 남아 있다. 이는
  Google 로그인 UI와 관리자 Google 계정의 2단계 인증 수행 자체를 증명하지
  않으며 reviewer 전용 권한의 추가 브라우저 검증은 아직 남아 있다.
...
- 다음 6곳 조사에서는 서울·경기와 공식 최소 사실 필드를 충족한 4곳만 비공개
  ...
  미등록이며, 신규 4건의 소재지·업무 분야·대표 출처와 공개 적합성은 아직
  승인하지 않았다.
