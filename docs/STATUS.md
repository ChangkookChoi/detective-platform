...
서울·경기 약 100개 탐정사무소를 대상으로 하는 정보 디렉터리 MVP의 공개 탐색과 초기 관리자 검수 흐름을 구현하고 있습니다. 데이터 계층, 공개 전환, 홈·목록·상세, Clerk 인증 검수, 개인정보 최소화 일별 집계에 이어 승인된 출처만 연결할 수 있는 Python 수집·변경 감지 기반을 구현했습니다. PostgreSQL 17 합성 데이터로 수집 후보부터 관리자 검수 경계와 기존 운영값 불변까지 통합 검증했으며, 지속형 개발·운영 DB, 실제 출처 정책·업체 데이터와 운영 인증 키는 준비 전입니다.
...
1. 신규 수집 후보의 운영 업체 생성과 `approved_with_edits` 승인 편집 흐름 구현
2. 정보 오류 신고·정정 요청 진입점과 운영 처리 흐름 구현
3. 실제 출처 후보의 이용 조건·robots를 확인하고 소규모 허용 출처 어댑터를 등록
4. 운영 Clerk 프로젝트·MFA·관리자 계정과 지속형 DB 준비 후 실제 인증 흐름 검증
5. 초기 업체 약 100곳의 출처 확인 및 수동 검수
6. 접근성, SEO, 보안, 브라우저 시각·상호작용과 운영 체크리스트 기반 출시 검증
