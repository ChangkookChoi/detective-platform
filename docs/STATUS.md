...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `pending` 5건, 공개 업체 1건
...
유입 경로를 구현하고 J&K·정의·원픽·엠디탐정 본점·수원점 5건을 `pending`
후보로 적재했습니다. 이용 안내·개인정보 처리방침 초안·광고 표시 정책과
...
1. 새로 등록한 J&K·정의·원픽·엠디탐정 본점·수원점 5건의 원문·소재 지역·
   관리형 업무 분야를 대조하고 승인·보류·반려 결정
2. 남은 10개 `manual_candidate`의 공식 원문·한 개 사무소·대표번호를 순차
   재확인하고 유효한 후보를 수동 등록
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자
  ...
  전용 권한과 보류 분기의 추가 브라우저 검증은 아직 남아 있다. 새 지역·출처
  입력 UI는 인증 페이지의 접근성 트리에서 레이블·활성 상태·선택지 구성을
  확인했지만, 추가한 Playwright 상호작용 assertion은 로케이터가 두 차례
  어긋나 역할 기반 로케이터로 교정한 뒤 최종 재실행하지 않았다.
