...
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `approved` 1건·
  공식 출처 수동 후보 `approved` 5건·`pending` 8건, 공개 업체 6건
...
현재 접근에 실패한 다해 화성 본사와 공식 HTML 내부 주소가 충돌하는 명진은
등록하지 않고 `deferred`로 전환했습니다. 인증
...
1. 새 수동 후보 8건의 공식 원문을 다시 대조해 최하위 소재 지역·관리형 업무
   분야·대표 출처 유형을 지정하고 승인·보류·반려 결정. 승인 시에도 제안값을
   그대로 신뢰하지 않고 업체별 근거와 공개 상세를 별도 확인
2. 다해 화성 본사의 접속 복구와 명진 공식 주소 정합성을 나중에 재확인하고,
   복구·정정된 경우에만 최소 사실 필드를 처음부터 다시 검수
3. 리앤장 DNS와 호시탐탐 홈페이지 복구 여부를 나중에 재확인하고, 복구된
...
- 2026-08-10 추가 등록 후보 중 다해 화성 본사는 공식 페이지·robots 직접
  ...
  푸터 주소가 충돌해 등록하지 않았다. 신규 8건은 아직 `pending`이므로 소재
  지역·업무 분야·대표 출처 검수와 공개 승인을 완료한 업체가 아니다.
