...
- 단계: 핵심 MVP 기능 구현
- 배포 상태: 미배포
- 데이터 상태: 실제 파일럿 결함 후보 `rejected` 1건·교정 `pending` 1건,
  공개 업체 0건
...
저장했으며, 2026-08-08 주소 결함 사유로 최종 반려했습니다. 감사 처리자 ID
일치와 운영 업체 0건도 다시 확인했습니다. 주소
지역명 중복 정규화를 `jsonld-v2`로 교정해 최소 권한 재수집을 실행하고
교정된 `pending` 후보 1건을 추가했으며 여전히 운영 업체 0건입니다. 추가 공식
홈페이지 후보를 다섯 차례에 걸쳐 추가 조사했고, 그중 16곳을 자동 수집 없이
...
1. 교정된 `jsonld-v2` 제안값과 공식 원문·지역·업무 분야를 사람이 대조하고 승인
   또는 보류 결정
2. 입력 준비를 재확인한 리앤장·호시탐탐을 수동 후보 화면으로 등록하고
   브라우저 폼·중복 안내·감사 정보 표시 확인
3. 입력 준비를 재확인한 J&K·정의·원픽과 엠디탐정 본점·수원점을 주소별
   수동 후보로 등록하고 중복·감사 정보 확인
4. 초기 업체 약 100곳의 출처 확인 및 수동 검수
5. 운영 PostgreSQL 공급자·네트워크·자격 증명을 결정하고 실제 자동 백업,
...
- 등록 파일럿 한 곳은 실제 이용 조건 표시·robots·구조를 확인하고 최소 권한
  ...
  보류 후 반려했고 교정된 `jsonld-v2` 후보를 별도로 생성했다. 교정 후보의
  사람 최종 승인, 운영 DB 자격 증명·네트워크 경계, 예약 실행과 알림은 아직
  검증하지 않았다.
...
- 공식 출처 수동 후보 등록·중복 안내·반려 결정은 실제 allowlist 관리자
  테스트 세션과 production Chrome·임시 PostgreSQL에서 통과했다. reviewer
  전용 권한, 보류·공개 승인 분기와 접근성의 수동 시각 검증은 아직 남아 있다.
