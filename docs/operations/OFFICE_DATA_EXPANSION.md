...
서울·경기·인천 공개 업체를 약 100곳까지 확대하고 전국 비공개 후보를 500곳 이상
확보할 때 후보 발굴, 중복 검사, 정책 사전검증, 관리자 등록·승인과 결과 검증을
반복 가능한 배치로 수행한다. 전국 후보 목표는 공개 목표가 아니며 자동 수집 또는
운영 스크립트가 관리자 승인 없이 업체를 공개하지 않는 기존 경계는 유지한다.
...
3. filtered JSONL은 서울·경기·인천, 강한 탐정업 상호·NAVER 분류, 무관 업종·오락성
   부정 문맥, 실행 내부 중복, DB·출처 등록부 중복과 공식 링크 필요 여부를 사유
   코드로 기록한다. 단순 `탐정` 부분 문자열만 있는 결과는 자동 통과시키지 않는다.
...
8. `content_check_required`만 정책 기반 HTTP client로 읽고 원문 파일을 남기지
   않는다. JSON-LD와 보이는 HTML에서 상호·업무용 대표번호·주소와 탐정 업무
   문구를 추출해 강한 일치·부분 일치·정보 부족으로 분류한다. 상호·주소·전화가
   맞더라도 탐정 업무 증거가 없으면 최종 검토 큐에 넣지 않는다.
9. 여러 facts 파일은 `build-discovery-review-queue`로 업체 정체성 해시 기준
   중복 제거한다. `pending` 검토 큐에는 관련성 probable, 강한 사실 일치, 대표
...
웹문서 API 재검색 없이 probe 단계에 연결한다. 기본 실행은 계속 서울·경기·
인천만 허용하며 전국 후보도 공개 지역 seed·제품 범위가 확장되기 전에는
비공개로 둔다.
...
`data/private` 아래의 `office-discovery-review-v3` JSONL이어야 하고 권한은
...
적재 뒤에는 `/admin/reviews`에서 원문 링크와 공식 출처에서 추출한 상호·주소·
대표전화를 대조하고, 주소에서 자동 제안된 최하위 소재 지역과 직접 확인되는
업무 분야를 선택하고 자동 생성된 공개 slug를 확인해 개별
