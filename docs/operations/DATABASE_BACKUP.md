...
PR #9 병합 후 실제 수동 backup run `31667893789`에서 이 guard의 GitHub API
권한과 업로드 직후 조회를 검증했다. 사전 검사는 활성 artifact 3개·182,895바이트,
사후 검사는 4개·243,860바이트를 기록했다. 생성된 `age` archive는
60,101바이트, 업로드 artifact는 60,965바이트이며 만료 시각은
2026-08-27 04:42 UTC(2026-08-27 13:42 KST)다. 전체 workflow는 51초에 모든
단계를 통과했다.

해담 증분 승격 직전 실제 수동 backup run `31675524560`은 Production 공개
업체 31건 상태에서 read-only dump를 수행했다. 평문 dump는 runner 임시
디렉터리에만 두고 60,528바이트 `age` archive와 manifest·비민감 메타데이터를
묶은 61,392바이트 artifact 한 건만 업로드했다. 만료 시각은 2026-08-27
06:53 UTC(2026-08-27 15:53 KST)이며 생성·만료 간격 14일을 확인했다. 사후
guard는 활성 백업 artifact 5개·총 305,252바이트를 기록했고 전체 workflow는
모든 단계를 통과했다. 이 artifact는 31→32 증분 승격 직전 복구 지점이며 실제
격리 복원은 기존 검증된 복원 run의 증거를 유지하고 별도 필요 시 실행한다.
