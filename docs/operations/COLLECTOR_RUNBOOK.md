...
NAVER 제한적 파일럿은 개별 단계를 임의 순서로 반복하지 않고
[`../../scripts/run-naver-discovery-cycle.sh`](../../scripts/run-naver-discovery-cycle.sh)를
사용한다. 스크립트는 지역 발견, 직접 공식 링크 우선 처리, 제한된 웹 보강,
정책·네트워크 probe, 최소 사실 추출, 검토/추가 조사 분리와 사유별 보강을
순서대로 실행한다. 실제 후보는 `data/private`에만 저장하고 출력에는 단계별
건수와 사유 집계만 남긴다. 실패한 단계는 다음 단계로 숨기지 않는다.

이 반복 실행은 NAVER 약관 재검토 기한, API 실행당 100회, 공식 사이트 후보당
최대 3개 보강 페이지, 관리자 승인 없는 공개 금지 경계를 변경하지 않는다.
