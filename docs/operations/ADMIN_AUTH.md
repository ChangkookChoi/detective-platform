...
- 같은 실제 Clerk 관리자 테스트 세션에서 공개 정정 요청을 접수하고 관리자
  상세 화면의 운영자 확인 출처를 별도 URL로 교체해 승인했다. 요청자가 제안한
  URL은 출처로 저장되지 않고 직접 확인한 URL만 비대표 출처·변경 필드 근거로
  연결되며, 승인 감사 처리자·출처 스냅샷과 공개 상세 변경을 임시 PostgreSQL·
  production Chrome에서 확인했다.
...
- 2026-08-13 신규 공식 출처 묶음에서 중복·접근 차단·DNS/TLS 장애·공식 주소
  충돌을 먼저 제외하고 럭스탐정사무소 서울 본사 1건만 같은 Clerk 공식 testing
  token과 실제 allowlist 관리자 세션으로 등록·승인했다. 실제 관리자 신규 후보
  폼과 상세 폼의 Server Action을 사용하고 사용자 위임 승인 사유를 남겼다.
  Clerk token 준비 1건과 production Chrome 관리자 흐름 1건이 통과했다. 독립
  DB 점검에서 유효한 제출자·승인 처리자, 대표 출처·필드 근거 6건, 공개 상세
  HTTP 200과 브라우저 오류 0건을 확인했다.
  최종 수동 후보는 승인 29건·수정 승인 1건·보류 1건·대기 0건, 지속형 개발 DB
  공개 업체는 31건이다. SQL 직접 삽입이나 인증·권한 우회는 사용하지 않았고
  일회성 자동화는 검증 후 제거했다.
- 같은 날 해담 탐정사무소 1건도 현재 공식 원문의 최소 사실 필드와 보수적 업무
  분야를 재확인한 뒤 같은 Clerk testing token·allowlist 관리자 세션의 실제
  신규 후보 폼과 승인 상세 폼으로 등록·승인했다. 제출자·승인 처리자가 현재
  allowlist에 있고 서로 일치하며 대표 출처 1건·근거 5건·업무 분야 2건,
  공개 상세 HTTP 200과 브라우저 오류 0건을 확인했다. 최종 수동 후보는 승인
  30건·수정 승인 1건·보류 1건·대기 0건, 지속형 개발 DB 공개 업체는 32건이다.
